...
export default function AdminHome() {
  ...
  return (
    <div>
      <h1 className="text-xl sm:text-2xl font-black text-gray-800 mb-1">Resumen</h1>
      <p className="text-gray-500 text-xs sm:text-sm mb-5 sm:mb-6">Vista general de Sonrisas.</p>

      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-2.5 sm:gap-3 mb-6 sm:mb-8">
        {cards.map(c => (
          <Link key={c.label} href={c.href}
            className={`rounded-2xl p-3 sm:p-4 text-white shadow-sm bg-gradient-to-br ${c.color} hover:scale-[1.02] transition-transform min-w-0`}>
            <p className="text-white/80 text-[10px] sm:text-xs font-semibold uppercase tracking-wide truncate">{c.label}</p>
            <p className="text-2xl sm:text-3xl font-black mt-1 leading-none">{c.value ?? '—'}</p>
            <p className="text-white/70 text-[10px] sm:text-xs mt-1 truncate">{c.sub}</p>
          </Link>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 sm:gap-5">
        <div className="bg-white rounded-2xl p-4 sm:p-5 border border-gray-100">
          <div className="flex items-center justify-between mb-3 gap-2">
            <h2 className="font-black text-gray-800 text-sm sm:text-base">Últimos registros</h2>
            <Link href="/admin/usuarios" className="text-brand-500 text-xs font-bold whitespace-nowrap">Ver todos →</Link>
          </div>
          {ultimos.length === 0 ? (
        ...
        <div className="bg-white rounded-2xl p-4 sm:p-5 border border-gray-100">
          <h2 className="font-black text-gray-800 text-sm sm:text-base mb-3">Accesos rápidos</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {[
              ...
              <Link key={x.label} href={x.href}
                className="rounded-xl p-3 bg-gray-50 hover:bg-brand-50 transition-colors text-sm font-semibold text-gray-700 flex items-center gap-2 min-w-0">
