...
export default function ChatSonrisas({ mode = 'floating' }: { mode?: 'floating' | 'card' }) {
  ...
  // Cargar consultas del padre + flag de no-leído
  ...
  // Realtime: recargar al recibir respuesta nueva
  useEffect(() => {
    if (!userId) return
    const ch = supabase
    ...
    <>
      ...
      {abierto && (
        <div className="fixed inset-0 z-50 sm:inset-auto sm:bottom-24 sm:right-4 sm:w-[400px] sm:max-h-[70vh] sm:rounded-3xl bg-white shadow-2xl flex flex-col overflow-hidden border border-brand-100">
          ...
          <div className="bg-gradient-to-r from-brand-500 to-brand-700 text-white p-4 flex items-center gap-3 flex-shrink-0">
            ...
            {vista === 'lista' && (
              <button onClick={() => setVista('nueva')}
                className="bg-white/20 hover:bg-white/30 text-white text-xs font-black px-3 py-1.5 rounded-full">
                + Nueva
              </button>
            )}
              ...
              <>
                ...
                {!cargando && consultas.length === 0 && (
                  <div className="text-center py-6">
                    ...
                    <button onClick={() => setVista('nueva')}
                      className="bg-brand-500 text-white font-black px-5 py-2.5 rounded-2xl text-sm">
                      Hacer una pregunta
                    </button>
                  </div>
                )}
                    ...
                    {consultas.map(c => (
                      <button key={c.id} onClick={() => abrirConsulta(c)}
                        ...
                        <div className="flex items-start justify-between gap-2 mb-1">
                          <p className="font-bold text-brand-800 text-sm truncate flex-1">
                            {c.asunto || 'Consulta'}
                          </p>
                        ...
                        {c.ultimo_mensaje && (
                          <p className="text-brand-500 text-xs line-clamp-2">{c.ultimo_mensaje}</p>
                        )}
          ...
          {/* Footer / input — sólo en hilo o nueva */}
